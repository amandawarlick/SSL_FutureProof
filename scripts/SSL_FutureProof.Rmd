...
params <- c('int.p1', 'int.p2', 'int.p3', 'int.p4', 'int.p5', 'int.p6', 'int.p7', 'int.p8', 
            ...
            'phiP.prob', 'phiPM.prob', 'phi1.prob', 'phi1M.prob', 'phi2.prob', 'phi2M.prob',
...
phi.vals <- post_sum %>%
  ...
  transform(age = factor(age, levels = c('P', '1', '2', '3', '4', '5', '6', '7', '8', '9', 'A'))) %>%
  merge(mean.phi.vals, by = c('age'), suffixes = c('', '.mean'))
...
ggplot(phi.vals %>% filter(sex == 'Female' & region == 2),
  ...
  geom_line(aes(year, med.mean), col = 'grey60', linetype = 'dotted') +
  xlab('') + ylab(expression(paste('Predicted survival probability ',  '(', phi, ')'))) +
  ggtitle('') +
    facet_grid(. ~ age) +
...
#temporal variance
p.pars <- c('p1', 'p2', 'p3', 'p4', 'p5', 'p6', 'p7', 'p8', 'p9', 'pA')

p.pars <- all_pars[which(variable %in% p.pars)]

p.probs <- post_sum %>% filter(grepl('p1', variable))
