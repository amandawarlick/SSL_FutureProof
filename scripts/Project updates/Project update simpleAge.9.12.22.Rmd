...
Survival rates were estimated for ages P, 1:3, juveniles (ages 4-6) adults (ages 7+), with fixed effects estimating the effect of pup mass and the differences between sex and natal region on survival over 19 years of data. The sample sizes for natal region were imbalanced, with more samples from Pyramid Rock (*n* = 1395) than South Seal Rock (*n* = 290). Fixed effects of sex were estimated for each age group while those of natal region were estimated for pups, yearlings, shared for juveniles (ages 3-6) and adults (ages 7+). Temporal variance was estimated using a random effect of year with a penalized complexity prior, with the random effect estimated for pups, yearlings, and pooled for juveniles (ages 3-6) and adults (ages 7+) jointly for both sex and natal region (i.e., trends were not estimated for each sex nor region but for age groups in the eDPS as a whole). Detection rates were estimated for each age as above with temporal variance constructed in the same fashion, but detection was assumed to be constant across sexes, natal regions, and resight location.  
