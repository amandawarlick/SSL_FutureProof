...
### Project goal    
The goal of this project was to examine model performance in the estimation of Steller sea lion survival rates across a range of mark-recapture survey designs.  

### Simulation analysis  

#### Model framework  
Mark-recapture data were simulated over 10- and 20-year study horizons based on empirical age-specific survival and detection probabilities estimated for the eastern DPS. These data were fit to a Cormack Jolly Seber mark-recapture model framework to estimate survival of five age classes (pups, age-1, age-2, juveniles (ages 3-4), and adults 5+). Temporal variance was included (based on empirical estimates from the dDPS) for age class-specific survival probabilities, but detection probabilities were assumed constant over the study period. Uninformed priors were used for the estimation of detection probabilities and mean intercepts for survival parameters and penalized complexity priors with a fixed shrinkage rate were used to estimate temporal variance. More details about data generation, model framework, and Bayesian implementation to follow in formal write-up. 

#### Scenarios

The above model was fit using simulated data across 45 scenarios with varying branding cohort sizes (N = 50, 75, 100, 150, 200), branding frequency (annual, biennial, triennial), and detection rates (low, medium, high). Low and high detection rates were set at +/- 20% of the medium (based on empirical estimates) (e.g., for pups, detection rates were 0.48, 0.6, 0.72). Resighting effort was assumed to occur annually regardless of branding schedule based on discussions about the likely future reliance on remote cameras. Therefore, this could be seen as a "best case" scenario with resighting effor each year. 

#### Model performance  

Model performance was examined using relative bias, RMSE, coefficient of variation (CV), and the probability of detecting a change in survival over both 10- and 20-year study horizons. The 20-year study horizon can be thought of as being a representation of a situation where you have a population that already includes marked individuals (as is the case for Stellers) rather than starting a new 10-year study. 
...
tot_runs <- converged %>%
  filter(!is.na(rhat)) %>%
  transform(type = ifelse(batch %in% c('n7', 'n9'), 'Small change', 'Big change')) %>%
...
### Results  
Results for each of the model performance metrics is below. In general, it is important to note that the estimation of pup and adult survival often show the strongest patterns across the scenarios, but likely for two different reasons. Pup survival is intentionally simulated to have higher variability to reflect reality, and therefore may have poorer precision or increased bias. Adult 5+ survival, on the other hand, has minimal simulated variability but will always be informed by fewer branded cohorts, which can be important in the shorter study time horizons (e.g., adult survival would only be informed by 2 cohorts in a 10-year study). 

#### Big-picture take-aways  
- Results are consistent across performance metrics, though the strength of the apparent patterns varies across metrics.  
- Model performance degrades most notably across branding frequency, cohort sampling sizes, and study horizons, with smaller differences across detection levels.  
- Precision goals stated in MMPA permits can be reliably achieved with either a 20-year study horizon or annual branding. 
...
cv.files <- c(cv.files7, cv.files8, cv.files9, cv.files10)
...
##### Fewer facets: detecting changes at medium detection  
...
```{r crew fewer facets II, fig.width = 9, fig.height = 4}
