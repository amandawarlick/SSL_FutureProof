...
```{r results bias prepop, fig.width = 7, fig.height = 6}
...
ggplot(dotplot_dat %>% filter(N %in% c(50, 100, 200) & length == 20 & variable %in% c('phiP')),
  ...
  facet_nested(samp_size + N ~ detection, drop = T,
             labeller = labeller(survey_freq = freq_labels, length = occ_labels),
             scales = 'free_x') +
...
```{r results rmse prepop, fig.width = 7, fig.height = 6}
...
cv.files <- c(cv.files6, cv.files7, cv.files8, cv.files9, cv.files10)
