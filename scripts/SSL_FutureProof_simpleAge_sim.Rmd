...
cv <- dif_low_little <- dif_high_little <- dif_low_big <- dif_high_big <- matrix(NA, nsim, length(phi.pars)) 
...
### Priors
   ...
   for (t in 1:(n.occasions - 1)) {
    ...
    # sd.phiP <- sd(phiP[1:n_ind,1:(n.occasions-1)])
    # sd.phi1 <- sd(phi1[1:n_ind,2:(n.occasions-1)])
    # sd.phi2 <- sd(phi2[1:n_ind,3:(n.occasions-1)])
    # sd.phiJ <- sd(c(phi3[1:n_ind,4:(n.occasions-1)], phi4[1:n_ind,5:(n.occasions-1)]))
    # sd.phiA <- sd(c(phi5[1:n_ind,6:(n.occasions-1)], phi6[1:n_ind,7:(n.occasions-1)]),
    #               c(phi7[1:n_ind,8:(n.occasions-1)], phi8[1:n_ind,9:(n.occasions-1)]),
    #               c(phi9[1:n_ind,10:(n.occasions-1)], phiA[1:n_ind,10:(n.occasions-1)]))
...
# dif_low_little[s,] <- phi.pars-phi.pars*c(0.15,0.15,0.15,0.10,0.05) >= summary(out)$q[cri_pars,'2.5%'] & 
#   phi.pars-phi.pars*c(0.15,0.15,0.15,0.10,0.05) <= summary(out)$q[cri_pars,'97.5%']
# colnames(dif_low_little) <- phi.pars.names
# 
# dif_high_little[s,] <- phi.pars+phi.pars*c(0.15,0.15,0.15,0.10,0.05) >= summary(out)$q[cri_pars,'2.5%'] & 
#   phi.pars+phi.pars*c(0.15,0.15,0.15,0.10,0.05) <= summary(out)$q[cri_pars,'97.5%']
# colnames(dif_high_little) <- phi.pars.names
# 
# dif_low_big[s,] <- phi.pars-phi.pars*c(0.2,0.2,0.2,0.15,0.10) >= summary(out)$q[cri_pars,'2.5%'] & 
#   phi.pars-phi.pars*c(0.2,0.2,0.2,0.15,0.10) <= summary(out)$q[cri_pars,'97.5%']
# colnames(dif_low_big) <- phi.pars.names
# 
# dif_high_big[s,] <- phi.pars+phi.pars*c(0.2,0.2,0.2,0.15,0.10) >= summary(out)$q[cri_pars,'2.5%'] & 
#   phi.pars+phi.pars*c(0.2,0.2,0.2,0.15,0.10) <= summary(out)$q[cri_pars,'97.5%']
# colnames(dif_high_big) <- phi.pars.names

# dif_low_little[s,] <- phi.pars-phi.pars*c(0.15,0.15,0.15,0.10,0.05) <= summary(out)$q[cri_pars,'5%']
# colnames(dif_low_little) <- phi.pars.names
# 
# dif_high_little[s,] <- phi.pars+phi.pars*c(0.15,0.15,0.15,0.10,0.05) >= summary(out)$q[cri_pars,'95%'] 
# colnames(dif_high_little) <- phi.pars.names
# 
# dif_low_big[s,] <- phi.pars-phi.pars*c(0.2,0.2,0.2,0.15,0.10) <= summary(out)$q[cri_pars,'5%'] 
# colnames(dif_low_big) <- phi.pars.names
# 
# dif_high_big[s,] <- phi.pars+phi.pars*c(0.2,0.2,0.2,0.15,0.10) >= summary(out)$q[cri_pars,'95%'] 
# colnames(dif_high_big) <- phi.pars.names

dif_low_little[s,] <- phi.pars-phi.pars*c(0.15,0.15,0.10,0.05,0.05) <= 
        c(quantile(out_mat[,'int.phiP'], probs = 0.05), 
          quantile(out_mat[,'int.phi1'], probs = 0.05),
          quantile(out_mat[,'int.phi2'], probs = 0.05),
          quantile(out_mat[,'int.phiJ'], probs = 0.05),
          quantile(out_mat[,'int.phiA'], probs = 0.05))
colnames(dif_low_little) <- phi.pars.names

dif_high_little[s,] <- phi.pars+phi.pars*c(0.15,0.15,0.10,0.05,0.05) >= 
        c(quantile(out_mat[,'int.phiP'], probs = 0.95), 
          quantile(out_mat[,'int.phi1'], probs = 0.95),
          quantile(out_mat[,'int.phi2'], probs = 0.95),
          quantile(out_mat[,'int.phiJ'], probs = 0.95),
          quantile(out_mat[,'int.phiA'], probs = 0.95))
colnames(dif_high_little) <- phi.pars.names

dif_low_big[s,] <- phi.pars-phi.pars*c(0.2,0.2,0.15,0.10,0.10) <= 
  c(quantile(out_mat[,'int.phiP'], probs = 0.05), 
          quantile(out_mat[,'int.phi1'], probs = 0.05),
          quantile(out_mat[,'int.phi2'], probs = 0.05),
          quantile(out_mat[,'int.phiJ'], probs = 0.05),
...
dif_high_big[s,] <- phi.pars+phi.pars*c(0.2,0.2,0.15,0.10,0.10) >= 
  c(quantile(out_mat[,'int.phiP'], probs = 0.95), 
          quantile(out_mat[,'int.phi1'], probs = 0.95),
          quantile(out_mat[,'int.phi2'], probs = 0.95),
          quantile(out_mat[,'int.phiJ'], probs = 0.95),
...
saveRDS(dif_low_little, file = here::here('results', 'simpleAge', 'sims', 
                                      paste('S.', scenario,  '_low_little', batch, '.rds', sep = '')))
saveRDS(dif_high_little, file = here::here('results', 'simpleAge', 'sims', 
                                      paste('S.', scenario,  '_high_little', batch, '.rds', sep = '')))
saveRDS(dif_low_big, file = here::here('results', 'simpleAge', 'sims', 
                                      paste('S.', scenario,  '_low_big', batch, '.rds', sep = '')))
saveRDS(dif_high_big, file = here::here('results', 'simpleAge', 'sims', 
                                      paste('S.', scenario,  '_high_big', batch, '.rds', sep = '')))
...
cv.files <- c(cv.files3, cv.files4)
...
cri.scenarios <- c('high_big', 'high_little', 'low_little', 'low_big')
...
cri.files <- list.files(path = here::here('results', 'simpleAge', 'sims'), 
                    pattern = cri.scenarios[j], all.files = FALSE, full.names = F)
...
rhats <- data.frame(rhat = readRDS(file = here::here('results', 'simpleAge', 'sims',
                                                     gsub(cri.scenarios[j], 'median', cri.files[f])))[,'max_rhat'])
...
mean.cri <- data.frame(cri.all) %>%
  ...
  filter(run > 6) %>%
  group_by(variable, scenario, threshold) %>%
  dplyr::summarize(value = mean(value), .groups = 'keep') %>%
  merge(scenarios, by = 'scenario', all = F) %>%
  group_by(threshold, variable, detection, N, survey_freq) %>%
  dplyr::summarize(value = mean(value), .groups = 'keep') %>%
  transform(variable = factor(variable, levels = c('phiP', 'phi1', 'phi2', 'phiJ', 'phiA',
                            'p1', 'p2', 'pJ', 'pA'))) %>%
  transform(detection = factor(detection, levels = c('Low', 'Med', 'High'))) %>%
...
plot_dat <- mean.cri %>% filter(grepl('phi', variable)) %>%
  ...
  transform(threshold = factor(threshold, levels = c('low_little', 'low_big', 'high_little', 'high_big'))) %>%
  transform(threshold_lab = factor(threshold, 
                                   levels = c('low_little', 'low_big', 'high_little', 'high_big'),  
                                   labels = c('Little decrease', 'Big decrease', 'Little increase', 'Big increase')))

#categorical colors
ggplot(plot_dat %>% filter(threshold %in% c('high_little', 'high_big')), 
       # aes(x = factor(detection), y = variable, fill = value)) +
        aes(x = factor(detection), y = variable, fill = value_cat)) +
  geom_tile(color = 'grey50') +
  xlab('Detection') +
  ylab('Parameter') +
  facet_nested(samp_size + N ~ survey_frequency + survey_freq + threshold_lab, drop = T, 
             labeller = labeller(survey_freq = freq_labels),
             scales = 'free_x') +
# scale_fill_gradient2(name = "Probability",
#                   mid = "#FFFFFF", high = "deepskyblue1", low = "red2", midpoint = 0.95) +
  # scale_fill_discrete(name = "Probability of detecting change in survival") +
  theme_bw() +
  theme(legend.position = 'top',
             strip.background = element_rect(fill = "#EEEEEE", color = "#FFFFFF")) +
    scale_y_discrete("", labels = parse(text = c(eval(expression(levels(plot_dat$variable)))))) +
  scale_fill_manual(name = "Probability of detecting change in survival",
                    values = c('darkred', 'orange', 'dodgerblue2'))
...
ggplot(plot_dat %>% filter(threshold %in% c('low_little', 'low_big')), 
       aes(x = factor(detection), y = variable, fill = value)) +
        # aes(x = factor(detection), y = variable, fill = value_cat)) +
  geom_tile(color = 'grey50') +
  xlab('Detection') +
...
# plot_dat <- cri.all %>%
#   merge(scenarios, by = 'scenario', all = F) %>%
#   group_by(threshold, variable, detection, survey_freq, N) %>%
...
# 
# ggplot(plot_dat %>% filter(threshold %in% c('high_little', 'high_big')),
#        aes(x = N, y = value, col = detection, group = detection)) +
...
#   facet_grid(variable + threshold ~ survey_freq, label = 'label_parsed', scales = 'free') +
#   theme_bw() +
#   plot_theme(legend.position = 'top',
#              plot.subtitle = element_text(size = 10, hjust = 0.5, vjust = 1)) +
#   scale_color_manual(values = rainbow2[c(8,5,2)], name = 'Detection') 
# 
# ggplot(plot_dat %>% filter(threshold %in% c('high_little', 'high_big')),
#        aes(x = N, y = value, col = threshold, group = threshold)) +
#   geom_point() + geom_line() +
#   geom_hline(aes(yintercept = 0), linetype = 'dotted') +
#   xlab('Sample size') + ylab('Probability of detecting change') +
#     # facet_grid(variable~adult_p, labeller = labeller(variable = 'label_parsed'), scales = 'free') +
#   facet_grid(variable + detection ~ survey_freq, label = 'label_parsed', scales = 'free') +
#   theme_bw() +
#   plot_theme(legend.position = 'top',
#              plot.subtitle = element_text(size = 10, hjust = 0.5, vjust = 1)) +
