...
scenarios <- data.frame(scenario = c(sc),
                        survey_freq = c(5, 7, 8, 10),
                        detection = c('Med', 'Med', 'Med', 'Med'),
                        N = c(rep(50, 4)))
...
dotplot_low <- rel.bias %>% transform(length = 20) %>% 
  # bind_rows(rel.bias %>% transform(length = 10)) %>%
  merge(scenarios, by = 'scenario', all = F) %>%
  group_by(length, variable, detection, survey_freq, N) %>%
    dplyr::summarize(lower = quantile(value*100, probs = 0.025), .groups = 'keep')

dotplot_up <- rel.bias %>% transform(length = 20) %>% 
  # bind_rows(rel.bias %>% transform(length = 10)) %>%
  merge(scenarios, by = 'scenario', all = F) %>%
  group_by(length, variable, detection, survey_freq, N) %>%
...
# dotplot_dat_up <- cv %>% transform(length = 20) %>% 
...
#   filter(!is.na(variable))
# 
# dotplot_dat_low <- cv %>% transform(length = 20) %>% 
##   bind_rows(cv %>% transform(length = 10)) %>%
#   merge(scenarios, by = 'scenario', all = F) %>%
#   group_by(length, variable, detection, survey_freq, N) %>%
...
#study horizon
cv_cont_dot <- ggplot(dotplot_dat, aes(x = factor(survey_freq), y = value, col = factor(variable), group = variable)) +
  geom_point() + geom_line() +
...
cv_thresh_dot <- ggplot(dotplot_dat, aes(x = factor(survey_freq), y = value_cat, col = factor(variable), group = variable)) +
  geom_point() + geom_line() +
...
dotplot_dat <- cri.all %>% 
  merge(scenarios, by = 'scenario', all = F) %>%
  group_by(variable, detection, survey_freq, N, threshold) %>%
...
cri_dot_big <- ggplot(dotplot_dat,
       aes(x = factor(survey_freq), y = value, col = factor(variable), group = variable)) +
  geom_point() + geom_line() +
  geom_hline(aes(yintercept = 0.75), linetype = 'dotted') +
  xlab('Number of cohorts in 20 yrs') + 
...
cri_dot_big <- ggplot(dotplot_dat,
       aes(x = factor(survey_freq), y = value, col = factor(threshold), group = threshold)) +
  geom_point() + geom_line() +
  geom_hline(aes(yintercept = 0.75), linetype = 'dotted') +
  xlab('Number of cohorts in 20 yrs') + 
...
```
The spread of posterior medians gets smaller with greater number of cohorts.
