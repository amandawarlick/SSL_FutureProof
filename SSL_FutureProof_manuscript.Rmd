...
effort_dat <- cri_delta_steps %>%
  ...
  transform(value_2 = ifelse(cat == 'Other', 0, value)) %>%
  #modifying here
  transform(step_eqEff = ifelse(category == 'Detection & marking frequency' & survey_cat == 3, 6,
  ...
  transform(step_eqEff = ifelse(is.na(step_eqEff), 1, step_eqEff)) %>%
  ...
  transform(step_eff_label = factor(step_eqEff, levels = c(1, 2, 3, 4, 6), 
...
effort_dot_eff <- ggplot(effort_dat %>% filter(!is.na(step)) %>% filter(variable == 'Pup') %>%
         transform(samp_size = 'Study duration & cohort size') %>%
         transform(length = factor(length, labels = c('10 yrs', '20 yrs'))), 
       # aes(step_eqEff, value, group = category, col = category)) +
...
text_dat <- cri_delta_steps %>%
  ...
  transform(det_cat = factor(det_cat, labels = c('Low', 'Medium', 'High')),
            survey_cat = factor(survey_cat, labels = c('Triennial', 'Biennial', 'Annual'))) %>%
  transform(eff_cat = ifelse(step == 'Low', '1',
                        ifelse(survey_cat == 'Triennial' & det_cat == 'Medium', '2',
...
#heat map approach 
uneqCost_heat <- ggplot(effort_dat %>%  
...
eqEff_heat <- ggplot(effort_dat %>% 
                    filter(variable == 'Pup' & threshold == 'dec_big'), 
       aes(factor(step_eqEff), value, group = category, col = factor(step_eqEff))) +
  geom_line(col = 'grey') +
  geom_point(size = 2) +
  geom_hline(aes(yintercept = 0.75), linetype = 'dotted') +
  facet_nested(length~category+N) +
...
plot3 <- ggplot(effort_dat %>%  
  ...
  geom_point(aes(factor(step_eqEff), value, group = category, col = factor(step_eqEff)), size = 2, shape = 17, show.legend = F) +
  geom_line(aes(factor(step_eqEff), value, group = category, linetype = 'Equal cost'), col = 'darkgrey') +
  geom_hline(aes(yintercept = 0.75), linetype = 'dotted') +
  facet_nested(variable~category+N) +
...
plot4 <- ggplot(effort_dat %>%  
  ...
  #add equal effort dat
  geom_point(aes(factor(step_eqEff), value, group = category, col = factor(step_eqEff)), size = 2, shape = 17, show.legend = F) +
  geom_line(aes(factor(step_eqEff), value, group = category, linetype = 'Equal additive cost'), col = 'darkgrey') +
  geom_hline(aes(yintercept = 0.75), linetype = 'dotted') +
...
eqCost_grid <- ggplot(text_dat, aes(det_cat, survey_cat, fill = eff_cat)) +
  geom_tile(data =  text_dat, 
            color = "white", lwd = 1.5) +
  # geom_tile(data = text_dat %>% filter(category == 'Baseline'),
  #           fill = 'slateblue4', color = "white", lwd = 1.5, show.legend = F) +
  ...
  #           aes(label = step), show.legend = F) +
  # annotate('text', x = 1, y = 1, label = 'Baseline == 1', fontface = 'bold') +
  ylab('Marking frequency') + xlab('Detection') + ggtitle('Equal additive costs') +
  theme_bw() +
  theme(legend.position = 'none',
        legend.key.size = unit(0.35, "cm"),
        panel.grid.major = element_blank(),
...
uneqCost_grid <- ggplot(text_dat, aes(det_cat, survey_cat, fill = cost_cat)) +
  geom_tile(data =  text_dat, 
            color = "white", lwd = 1.5) +
  # geom_tile(data = text_dat %>% filter(category == 'Baseline'),
  #           fill = 'slateblue4', color = "white", lwd = 1.5, show.legend = F) +
  ...
  #           aes(label = step), show.legend = F) +
  # annotate('text', x = 1, y = 1, label = 'Baseline == 1', fontface = 'bold') +
  ylab('Marking frequency') + xlab('Detection') + ggtitle('Unequal additive costs') +
  theme_bw() +
  theme(legend.position = 'none',
        legend.key.size = unit(0.35, "cm"),
        panel.grid.major = element_blank(),
...
grids <- plot_grid(eqCost_grid, uneqCost_grid, nrow = 2)
